...
module.exports = function (io, Parse, Client, User, app) {
  ...
  function chatManager(socket) {
    ...
    function connect(data) {
      ...
      function handle(admin) {
         ...
         * */
        function reconnect() {
          Object.keys(chatEndpoints[ns].rooms)
        ...
        function connectUserFromQueue() {
          ...
          if (userSocket) {
            ...
            socket.join(newRoom);
            if (userSocket.tipId) {
              args.push(userSocket.username, userSocket.userObjectId,
                userSocket.tipId);
              addRoom(newRoom, socket, userSocket.username,
                userSocket.userObjectId);
        ...
        if (admin !== true) {
          socket.emit('no-access', {
            ...
            content: "User " + username +
              " doesn't have access to this action"
          });
        } else if (admin === true) {
      ...
      if (username != null && typeof username === 'string') {
        if (askingAdmin) {
          ...
          if (chatEndpoints[ns].nextAdmin > chatEndpoints[ns].activeAdmins
            .length - 1) {
    ...
    function routeMessage(data) {
      ...
      function sendPush(pObject) {
        return Parse.Push.send(pObject)
          .then(function () {
            console.log(
              'Successfully sent push notification to user %s',
              pObject.channels[0]);
            return "success";
          }, function (error) {
            if (pushTries < 5) {
      ...
      function sendMessage(message) {
        ...
        function pphHandleTipChatLog(chatLogs, tip) {
          ...
          if (chatLogs[0] != undefined) {
            ...
            return chatLog
              ...
              .set('controlNumber', tip.id + '-' + tip.get(
                'controlNumber'))
              .set('clientId', {
                __type: 'Pointer',
                className: 'Client',
        ...
        function logSuccess(chatLog) {
          console.log("Saved %s chat log", chatLog.get(
            'controlNumber'));
        }
      ...
      if (data.tipId && socket.admin === true && socket.room == undefined) {
        pushNotificationObject = {
          ...
          data: {
            alert: "An officer is trying to followup regarding this tip: " +
              data.tipId,
            badge: "Increment",
            sound: "cheering.caf",
            title: "Chat",
        ...
        sendPush(pushNotificationObject)
          .then(function () {
            getUsername(data.receiver)
              .then(function (u) {
                passPhrase = passwordGenerator.generatePassword(u);
                sendMessage(encryptionManager.encrypt(passPhrase,
                  data.message));
                new Parse.Query(TipReport)
                  .get(data.tipId)
                  .then(function (tip) {
                    if (tip) {
                      tip.set('hasMessage', true);
                      tip.save();
                    }
                  });
              });
    ...
    function disconnectChat() {
      ...
      function cFindAdminSetInactiveLog(adminSocket) {
        if (adminSocket.username === chatEndpoints[ns].rooms[socket.room]
          .admin) {
  ...
  app
    ...
    .post(auth, function (req, res) {
      ...
      function ppHandleTipRerpotQuery(tipObject, allLogs, logArray,
        encryptedMessages, controlNumber) {
        if (tipObject) {
          var username = tipObject.get('user')
            .get('username'),
            userId = tipObject.get('user')
            .id,
            messages = [],
            raw_crime = tipObject.get('crimeType')
            .base64,
            crimeType = encryptionManager.decrypt(passwordGenerator.generatePassword(
              username), raw_crime);
          ...
          if (encryptedMessages) {
            encryptedMessages.forEach(function (encryptedMessage) {
              var password = passwordGenerator.generatePassword(
                username);
              encryptedMessage.message = encryptionManager.decrypt(
                password, encryptedMessage.message);
              messages.push(encryptedMessage);
            });
          }

          addLog(controlNumber, tipObject.id, username, userId, messages,
            crimeType, logArray, allLogs);
      ...
      function addLog(controlNumber, tipObjectId, username, userId,
        messages, crimeType, logArray, allLogs) {
        allLogs.push({
      ...
      function ppHandleChatLogQuery(logs) {
        ...
        function checkIfListIsFullyDecrypted(decryptedArray,
          encryptedArray) {
        ...
        function iterateOverEncryptedLogs(log, logIndex, logArray) {
          ...
          new Parse.Query(TipReport)
            ...
            .get(tipObjectId)
            .then(function (tip) {
              return Parse.Promise.as(tip, allLogs, logArray,
                encryptedMessages, controlNumber);
            })
    ...
    .post(auth, function (req, res) {
      ...
      function pphHandleTipQueryStep2(tip) {
        return new Parse.Query(TipChatLog)
          .equalTo('controlNumber', tip.id + '-' + tip.get(
            'controlNumber'))
          .first()
          .then(function (chatLog) {
            if (chatLog) {
      ...
      function pphSetChatLogActive(chatLog, tipInfo) {
        return chatLog.set('active', true)
          .save()
          .then(function (chatLog) {
      ...
      function pphCreateChatLog(tipInfo) {
        if (tipInfo) {
          return new TipChatLog()
            ...
            .set('controlNumber', tipInfo.id + '-' + tipInfo.get(
              'controlNumber'))
            .save()
            .then(function (chatLog) {
              return Parse.Promise.as(chatLog, tipInfo)
        ...
        new Parse.Query(User)
          ...
          .then(function (username) {
            try {
              var password = passwordGenerator.generatePassword(
                username);
              var crimeTypeEncrypted = tipInfo.get('crimeType')
                .base64;
              var crimeType = encryptionManager.decrypt(password,
                crimeTypeEncrypted);
              ...
              return Parse.Promise.as(
                'Successfully added tip with controlNumber %s to chat queue',
                chatLog.get('controlNumber'));
            } catch (e) {
              console.warn(e.message);
