            ...
            if (room) {
                ...
                if (chatEndpoints[ns].activeAdmins.indexOf(socket) !== -1) {
                    getUsername(data.receiver)
                        .then(function (u) {
                            passPhrase = util.passwordGenerator.generatePassword(u);
                            sendMessage(util.encryptionManager.encrypt(passPhrase, data.message));
                        });
                } else {
                    getUsername(data.sender)
                        .then(function (u) {
                            passPhrase = util.passwordGenerator.generatePassword(u);
                            sendMessage(util.encryptionManager.decrypt(passPhrase, data.message));
                        });
                }
                                ...
                                tipChatObject.messages.forEach(function (message, index, array) {
                                    ...
                                    function decrypt(prop, cb) {
                                        ...
                                        if (passwords[message[prop]]) {
                                            ...
                                            var message = util.encryptionManager.decrypt(password, message.message);

                                            cb(message);
                                        } else {
                                            getUsername(message[prop], function (username) {
                                                password = util.passwordGenerator.generatePassword(username);
                                                passwords[message[prop]] = password;
                                                cb(util.encryptionManager.decrypt(password, message));
                                            });
                                        }
                                    }
                            ...
                            new Parse.Query(Parse.Object.extend('TipReport'))
                                ...
                                .then(function (tipObject) {
                                    ...
                                    encryptedMessages.forEach(function (encryptedMessage) {
                                        ...
                                        password = util.passwordGenerator.generatePassword(username);
                                        encryptedMessage.message = util.encryptionManager.decrypt(password, encryptedMessage.message);
