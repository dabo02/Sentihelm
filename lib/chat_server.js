...
module.exports = function (io, Parse, Client, User) {
    ...
    function chatManager(socket) {
        ...
        socket.emit('welcome', 'welcome the the chat service!');
        // event handlers
                ...
                if (askingAdmin) {
                    ...
                    if (adminSocket) {
                        adminSocket.join(room, function (err) {
                            ...
                            adminSocket.set('room', room);
                            adminSocket.emit('new-room', room, username);
                            socket.join(room);
                        ...
                        chatEvents.on('user-disconnect', function (user) {
                            if (user === socket) {
                                socket.leave(room);
                            }
                        });
        ...
        function  changeRoom(data) {
            ...
            if (admin) {
                socket.set('room', data.room, function (err) {
                   if(err) {
                       socket.emit('error', {
                           status: 500,
                           error: 'Internal Server Error',
        ...
        function disconnectChat() {
            ...
            chatEvents.emit('user-disconnect', socket);
            ...
            if (admin) {
                ...
                delete chatEndpoints[ns].activeAdmins[index];
            }
        }
        ...
        function test(data) {
            ...
            function respond() {
                socket.volatile.emit('test-back', response + ' ' + count);
                ...
                count += 1;
                timeout = setTimeout(respond, 5 * 1000);
            }
