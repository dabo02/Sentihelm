                    ...
                    if(log) {// Just add the message to the log array.
                      ...
                      try {
                        if(log.get('officerSocket')){
                          ...
                          else{ // officer is not current chatter
                            message.message = 'Another officer attempted to chat with this mobile user.';
                            socket.nsp.connected[officerSocket].emit('new-message', message, chatId);
                            message.message = 'There is another officer currently chatting with this mobile user.';
                            socket.emit('new-message', message, chatId);
                          }
                        }
            ...
            if(log) {
              ...
              if (status.status === 'mobileDropped') {
                ...
                if (officerSocket) {
                  ...
                  socket.nsp.connected[officerSocket].emit('mobile-disconnect', status.chatId);
                }
              ...
              if (status.status === 'mobilePaused') {
                status.message = 'Stream has been paused by mobile user';
                if (officerSocket) {
                  status.receiver = socket.nsp.connected[officerSocket].session.user.objectId;
                  socket.nsp.connected[officerSocket].emit('mobile-paused', status);
                }
              }
